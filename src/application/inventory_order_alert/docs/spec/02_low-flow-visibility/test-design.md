...
|---|--------|---------|-----------|--------|
| E-001 | クライアント配信ペイロードの増分 | 1 行あたりの `flowQuadrants` + `noIncomingRecord` + `responsibleDepartment` の JSON バイト数が **240 バイト以下**であること（2026/08/28 実測 222 バイト。当初の「100 バイト以下」は設計書 §3.2 の見積り誤り（JSON のキー名分を未計上）に基づいていたため、実測に合わせて改訂した） | NF-001 / R-3 | P2 |
