...
**配信サイズ（2026/08/28 実測値に更新）**: `flowQuadrants` + `noIncomingRecord` で
1行あたり **163 バイト**、`responsibleDepartment` を含めると **222 バイト**（UTF-8・区切り最小化時）。
当初は約 90 バイトと見積もっていたが、これは JSON のキー名と引用符の分を数えていなかった。
5,000 行でも約 1.1 MB であり、既存の `display` マップ（14 列分）に比べて十分小さいという
判断自体は変わらない。判定ロジックは domain の純関数1つに保たれる。
...
| R-3 | 6通りの事前計算でクライアントペイロードが増える | 一覧の初期表示が遅くなる | 増加は1行あたり **222 バイト**（2026/08/28 実測。当初見積り約90バイトは JSON のキー名分を数えていなかった）。既存の `display`(14列) に比べ十分小さい。NF-001 の性能要件に対して実測で確認する（テスト設計書に計測項目を置く） |
