...
### 実装後の調整（2026/09/17、ユーザー確認による）

| # | タスク | レイヤー | 状態 |
|---|--------|---------|------|
| 69 | 仕様の改訂（REQ-SFV-F-004/F-005、design §6.3、test-design X-004/006/015/018、機能仕様書 5.2） | docs | [✅2026/09/17 09:41] |
| 70 | `list.html` / `app.css` のセルを区分名のみに、入荷実績なしバッジ撤去 | interfaces | [✅2026/09/17 09:44] |
| 71 | `inventory-order-alert-list-client.js` / `inventory-order-alert-list.js` の追随 | interfaces | [✅2026/09/17 09:44] |
| 72 | テスト改修と全体検証 | interfaces/test | [✅2026/09/17 09:47] |
...
--------------------
### タスク69〜72 実行レポート（2026/09/17 09:47）
- 対象: ユーザー確認で「流動区分が長い。説明はクリックで出るように」「入荷実績なしマークは最終入荷日で分かるので不要」の指摘。AskUserQuestion で「常時表示は区分名のみ」「説明は詳細ダイアログに任せる」を選択
  - 69 `requirements.md`（F-004・F-005 に改訂前注記）、`design.md` §6.3、`test-design.md`（X-004/006/015/018）、機能仕様書 §4.1.5・§4.1.6・改訂履歴 5.2
  - 70 `list.html`（セルは `<span class="ioa-flow-quadrant">` のみ、`data-*` は詳細ダイアログ用に維持）、`app.css`（積み上げスタイル・バッジ・緊急度・狭幅ルールを削除）、キャッシュバスター `20260917-flow-cell-compact`
  - 71 `list-client.js`（`renderFlowCell` は区分名のみ。`flowStatusOf` / `urgencyTextOf` 等は詳細ダイアログ用 API として残す）、`list.js`（区分名横の「（入荷実績なし）」を削除）
  - 72 `test_inventory_order_alert_views.py`・`test_inventory_order_alert_list_js.py` を改訂。`pytest` 全体 **1,904 passed**、`manage.py check` 問題なし
- 結果: Green
- 懸念事項: 緊急度（在庫月数・在庫切れ予測月）が一覧から見えなくなった。並び替え「在庫月数」で順序は付けられるが、値を一覧で見るには詳細ダイアログを開く必要がある。必要なら「区分名＋緊急度」への戻しは 1 行の変更で可能
- 改善事項: 説明の表示先を詳細ダイアログに一本化したことで、状況・推奨アクションの描画コードが JS 1 か所（`fillDetailSections`）になった
- 設計の Good ポイント: 行の `data-*` 属性と `recommendedActions` ペイロードは変えていないため、表示先の変更がドメイン・API に波及していない
- チーム共有ポイント: 判定ルールダイアログ（アクション行の「判定ルール」）に 状況 / 推奨アクション の一覧表があるので、区分ごとの説明はそこでも読める
--------------------
