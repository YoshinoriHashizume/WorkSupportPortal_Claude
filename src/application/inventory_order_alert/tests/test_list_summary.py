...
def _query(
    *,
    ...
    flow_selection: FlowSelection = SELECTION_Y1,
) -> ListQuery:
    return ListQuery(
        as_of_date=AS_OF,
        cust_code=cust_code,
...
def _source_row(
    *,
    cust_code: str = "112",
    vend_code: str = "9209",
    last_incoming_date: str = "2025/01/10",
...
def _filter_row(
    *,
    cust_code: str = "112",
    vend_code: str = "9209",
    flow_quadrant: str = QUADRANT_LOW_FLOW_NO_INCOMING,
    confirmation_status: str = "未確認",
    qty: int = 100,
) -> dict[str, object]:
    return {
...
def test_apply_flow_quadrants_to_rows_adds_quadrant_key_matrix_and_flags():
    ...
    assert enriched[0]["flow_quadrant"] == QUADRANT_LOW_FLOW_NO_INCOMING
    assert enriched[0]["flow_quadrant_key"] == "low-flow-no-incoming"
    assert set(enriched[0]["flow_quadrants"]) == PERIOD_KEYS
    assert enriched[0]["flow_quadrants"]["Y1"] == "low-flow-no-incoming"
...
def test_apply_flow_quadrants_to_rows_uses_selected_evaluation_period():
    ...
    enriched = apply_flow_quadrants_to_rows(
        rows, as_of_date=AS_OF, query=_query(flow_selection=SELECTION_Y3)
    )
...
def test_apply_flow_quadrants_to_rows_keeps_existing_row_keys():
    ...
    assert enriched[0]["last_incoming_date"] == "2025/01/10"
...
def test_filter_summary_rows_keeps_only_selected_flow_quadrant():
    rows = [
        _filter_row(flow_quadrant=QUADRANT_LOW_FLOW_NO_INCOMING),
        _filter_row(flow_quadrant=QUADRANT_DORMANT_STOCK),
        _filter_row(flow_quadrant=QUADRANT_NORMAL_FLOW),
    ]
    filtered = filter_summary_rows(rows, _query(flow_quadrant="low-flow-no-incoming"))
...
def test_filter_summary_rows_with_attention_only_excludes_normal_flow():
    rows = [
        _filter_row(flow_quadrant=QUADRANT_LOW_FLOW_NO_INCOMING),
        _filter_row(flow_quadrant=QUADRANT_DORMANT_STOCK),
        _filter_row(flow_quadrant=QUADRANT_LOW_FLOW_NO_SHIPMENT),
        _filter_row(flow_quadrant=QUADRANT_NORMAL_FLOW),
    ]
...
def test_filter_summary_rows_combines_flow_quadrant_with_confirmation_filter_as_and():
    rows = [
        _filter_row(flow_quadrant=QUADRANT_LOW_FLOW_NO_INCOMING, confirmation_status="確認済み"),
        _filter_row(flow_quadrant=QUADRANT_LOW_FLOW_NO_INCOMING, confirmation_status="未確認"),
        _filter_row(flow_quadrant=QUADRANT_NORMAL_FLOW, confirmation_status="未確認"),
    ]
    filtered = filter_summary_rows(rows, _query(flow_quadrant="low-flow-no-incoming", hide_confirmed=True))
    assert len(filtered) == 1
    assert filtered[0]["flow_quadrant"] == QUADRANT_LOW_FLOW_NO_INCOMING
...
def test_filter_summary_rows_combines_flow_quadrant_with_customer_code_filter_as_and():
    rows = [
        _filter_row(cust_code="112", flow_quadrant=QUADRANT_LOW_FLOW_NO_INCOMING),
        _filter_row(cust_code="999", flow_quadrant=QUADRANT_LOW_FLOW_NO_INCOMING),
        _filter_row(cust_code="112", flow_quadrant=QUADRANT_NORMAL_FLOW),
    ]
    filtered = filter_summary_rows(rows, _query(cust_code="112", flow_quadrant="low-flow-no-incoming"))
    assert len(filtered) == 1
    assert filtered[0]["cust_code"] == "112"
    assert filtered[0]["flow_quadrant"] == QUADRANT_LOW_FLOW_NO_INCOMING
