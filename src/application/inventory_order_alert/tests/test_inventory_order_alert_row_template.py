...
def test_rows_for_template_sets_alert_row_class_to_flow_quadrant_key():
    rows = _rows_for_template(
        [
            {"flow_quadrant": QUADRANT_LOW_FLOW_NO_INCOMING, "confirmation_status": "未確認"},
            {"flow_quadrant": QUADRANT_LOW_FLOW_NO_SHIPMENT, "confirmation_status": "未確認"},
        ]
    )
    assert rows[0]["alert_row_class"] == "low-flow-no-incoming"
    assert rows[1]["alert_row_class"] == "low-flow-no-shipment"
