---
name: design-review-l2-codex
description: |
  機能設計書のL2レビュー（Codexによる独立第2意見レビュー）を実行する。
  Claudeで実施済みのDesign-L1（機能設計レビュー）を、別モデル（Codex / OpenAI）に
  独立して再検証させ、先行レビューの見落としをクロスチェックする。
  戦略的設計との整合性、ドメインモデルの妥当性、ビジネスルールの配置、
  ユビキタス言語との整合性を、Design-L1と同じ観点で網羅する。
  レビュー結果は design.md には追記せず、別レポートファイルに出力する。
  ユーザーが「Codexで設計書をレビューして」「設計書のL2レビューをお願い」
  「別モデルで設計書をレビュー」「Codexに設計書をクロスチェックさせて」
  などと依頼したときに使用する。
argument-hint: "[機能設計書のファイルパス] [任意: --model <codexモデル名>]"
allowed-tools: Read, Write, Glob, Grep, Bash(command:*), Bash(codex:*), Bash(date:*), Bash(mkdir:*), Bash(cat:*)
---

# 機能設計書 L2レビュー（Codexによる独立第2意見レビュー）

Claude で実施した Design-L1（機能設計レビュー）の後に、**別モデル（Codex / OpenAI）** に
Design-L1 の全4観点を網羅した独立レビューを実行させ、**独立した第2意見（クロスチェック）** を得るためのスキルです。

- このスキルの実行主体は **Claude（オーケストレーター）** であり、実際のレビューは `codex exec` で起動した Codex が行う。
- レビュー結果は **design.md には追記せず、別レポートファイル** に保存する。
- design.md の本文・レビュー履歴・その他の成果物は **一切変更しない**。
- 対象は **機能設計書（design.md）** のみ。テスト設計書（test-design.md）は対象外。

---

## 前提条件

- Claude 側で Design-L1 レビューを実施済みであること（本スキルはその後の独立チェック）。
...
- 第1引数を機能設計書のパスとして扱う。`--model <name>` が渡された場合は Codex モデル指定として控える。
- **引数が空の場合は、必ずユーザーに「どの機能設計書をレビューしますか?」と質問する。**
- パスのヒント: 機能設計書は通常 `application/{app_name}/docs/spec/{feature-name}/design.md`。
...
- 関連文書のパスを Glob で解決する（存在するものだけを Codex に渡す）:
  ...
  - 対応する要件定義書（design.md の「対応文書」欄に記載されたパス。通常 `application/{app_name}/docs/spec/{feature-name}/requirements.md`）
- **観点3（ビジネスルールの配置）用のアーキテクチャreference** を判定する:
  - CLAUDE.md のセクション1（Project Overview）の技術スタックを確認する。
  - Django / Python → `design-review-l1/references/django-clean-architecture.md`
  - Swift → `design-review-l1/references/swift-clean-architecture.md`
  - C# → `design-review-l1/references/csharp-clean-architecture.md`
  - 判定できない場合はユーザーに確認する。存在すれば本文を Codex に渡す（後述の埋め込み）。
...
1. このスキルの `references/combined-review-prompt.md` を Read で読み込む（Design-L1 の全4観点と出力フォーマットが自己完結で書かれている）。
2. その内容の末尾に、実行時の情報を追記した「実行時アペンディックス」を作る:
   - **「ツール・シェルを使わないこと。必要なファイル本文はこのプロンプト末尾に埋め込み済み」** と明記する。
   - **存在する関連文書の扱い**（strategic_design・ユビキタス言語集・アーキテクチャreference・要件定義書が無ければ該当観点をスキップ／一般原則で判断する旨）を明記する。
   - Claude 側の Design-L1 レビュー履歴がこの会話内にあれば、その要約を「先行レビューの結論」として添付し、
     「これに引きずられず独立に検証し、見落としを拾うこと」と指示する。
3. 次の順でプロンプトファイル（スクラッチパッド配下、例: `codex-design-review-prompt.md`）を組み立てる:
   1. `combined-review-prompt.md`（レビュー観点・出力フォーマット）
   2. 実行時アペンディックス（上記2）
   3. **対象 `design.md` の本文**（区切り: `===== BEGIN FILE: design.md =====` 〜 `===== END FILE ... =====`）
   4. **存在する関連文書（`strategic_design.md`・`ubiquitous_language.md`・アーキテクチャreference・`requirements.md` 等）の本文**（同様の区切りで埋め込む）
   - 区切りは三連バッククォートではなく `===== BEGIN/END FILE =====` 形式にする（対象文書内の code fence と衝突しないため）。
   - 組み立ては `cat` で連結するのが簡単（例: `{ cat REF; cat APPENDIX; printf '...BEGIN...'; cat DESIGN; printf '...END...'; } > PROMPT_FILE`）。
...
cat <PROMPT_FILE> | codex exec \
  ...
  [--model <name>] \
  - \
  > <OUT_FILE> 2>/tmp/codex-design-review.err
```
...
- 終了コードを確認する。非0なら `/tmp/codex-design-review.err` の内容とともにユーザーに報告し、原因（未認証・モデル名誤り・ネットワーク等）を案内して中断する。
...
Codex の出力を、design.md とは別の専用レポートファイルに保存する。
...
2. 出力先ディレクトリを作成する:
   ...
   ```
3. レポートファイル名: `design-review-codex-YYYYMMDD-HHMM.md`
4. `<OUT_FILE>` の内容を Read し、先頭にメタデータヘッダを付けて Write で保存する:

````markdown
# 機能設計書 L2レビュー（Codex 独立第2意見）レポート

- **対象文書**: application/{app_name}/docs/spec/{feature-name}/design.md
- **レビュー種別**: Design-L1（機能設計）観点・独立クロスチェック
- **実行モデル**: <codex実行モデル名 / 未指定なら "Codex 既定モデル">
- **実行ツール**: Codex CLI (`codex exec`)
- **実行日時**: YYYY/MM/DD HH:MM
- **先行レビュー**: Claude による Design-L1（会話履歴／該当レポートを参照）
...
- **design.md 本体・レビュー履歴には追記しない。** 本スキルの成果物はこの別レポートのみ。
...
- **第2意見としての価値**を明確にする: Claude の Design-L1 レビュー結果と突き合わせ、
  **Codex だけが指摘した項目（先行レビューの見落とし候補）** を抜き出して提示する。
- 対応方針（どの指摘を design.md に反映するか）は **ユーザーが判断** する旨を伝える。
  反映が決まった指摘は、通常どおり Design-L1 スキルまたは手動で design.md に反映する。

---

## 注意事項

- このスキルは **既存ファイルを変更しない**（レポートの新規作成のみ）。design.md やユビキタス言語集の更新は行わない。
- Codex は **読み取り専用** で実行し、ファイル操作は Claude が担う（意図しない変更を防ぐ）。
- Codex の指摘はあくまで **第2意見**。Claude の Design-L1 と食い違う場合は、仕様（SSOT）・戦略的設計・ユビキタス言語集に照らして最終判断する。
- 本レビューは **機能設計（ドメイン設計）** が対象。実装コードのレイヤー違反・コーディング規約は対象外（実装レビューで扱う）。テスト設計書は対象外（テスト設計レビューで扱う）。
- Codex CLI 未インストール・未認証の環境では実行できない（手順1で検知）。

## 次のステップ

- Codex の指摘のうち反映すべきものを確定したら、Design-L1（機能設計レビュー）や手動修正で design.md に反映する。
- 反映後、必要なら再度このスキルで独立チェックを行い、レポートを更新（新しいタイムスタンプで別ファイル）してもよい。
- テスト設計書の独立レビューが必要な場合は、別途テスト設計レビュー相当の観点で実施する。
