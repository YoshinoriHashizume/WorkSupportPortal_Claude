...
from applications.portal.domain.menu_access import can_access_menu_item, receipt_comparison_menu_key
...
def test_menu_access_usecase_detects_admin_role():
    usecase = MenuAccessUsecase(FakeMenuAccessRepository(role_names={"管理者"}))
...
def test_favorites_usecase_skips_inaccessible_items():
    menu_access = MenuAccessUsecase(FakeMenuAccessRepository(groups={"production"}))
    usecase = FavoritesUsecase(DjangoFavoriteRepository(), menu_access)
