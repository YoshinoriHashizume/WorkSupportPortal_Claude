...
from applications.gonenkukumi.domain.errors import OracleQueryError
from applications.gonenkukumi.domain.result import build_multi_month_result, month_label
...
def test_search_usecase_saves_history():
    ...
    result = SearchUsecase(gateway, history).execute(1, {
        "custCode": "101",
        "custItem": "ITEM-001",
        "optionChange": "*",
        "yearMonth": "2026/05",
...
def test_search_page_usecase_returns_customers():
    context = SearchPageUsecase(FakeSearchGateway(), FakeHistoryRepository()).execute(1)
...
def test_result_page_usecase_builds_multi_month_result():
    ...
    result = ResultPageUsecase(gateway).execute({
        "custCode": "101",
        "custItem": "ITEM-001",
        "optionChange": "*",
        "yearMonth": "2026-05",
...
def test_list_history_usecase():
    histories = ListHistoryUsecase(FakeHistoryRepository()).execute(1)
...
def test_export_excel_usecase_returns_workbook_bytes():
    ...
    content, filename = ExportExcelUsecase(gateway).execute({
        "custCode": "101",
        "custItem": "ITEM-001",
        "optionChange": "*",
        "yearMonth": "2026-05",
