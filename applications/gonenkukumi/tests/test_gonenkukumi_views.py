...
from applications.portal.models import PortalMenuGroupAccess, UserFavoriteMenu
...
def test_search_api_hides_internal_naisak_not_found_code(client, user, monkeypatch):
    ...
    monkeypatch.setattr("applications.gonenkukumi.infrastructure.oracle.client.run_gonenkukumi_oracle_search", raise_not_found)
    client.force_login(user)
    response = client.post(
        "/api/gonenkukumi/search",
...
def test_result_page_initially_shows_base_month_and_add_buttons(client, user, monkeypatch):
    ...
    monkeypatch.setattr("applications.gonenkukumi.infrastructure.oracle.client.run_gonenkukumi_oracle_search", fake_search)
    client.force_login(user)
