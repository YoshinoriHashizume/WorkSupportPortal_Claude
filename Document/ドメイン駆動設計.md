...
- 各コンテキストごとに **用語集（グロッサリー）** を `src/application/<app>/docs/ubiquitous_language.md` で維持する。コンテキストを横断する用語は `src/docs/ubiquitous_language_core.md`（core）に置く。
...
| `src/application/<app>/docs/` | **アプリ固有の文書**（機能仕様書・テスト仕様書・`ubiquitous_language.md`・`spec/{feature}/`・`issues/`）。コードと同じ場所で改訂する |
| `src/docs/` | **プロジェクト横断の設計文書**（`ubiquitous_language_core.md`・`strategic_design.md`・アプリ横断の共通仕様） |
| `Document/` | **全体の手順書**とアプリケーション仕様書（リポジトリルート） |
...
| `asset_inventory` | 同上 | ListPage, ExportCsv, ResolveAccessKey, FetchAttachment | desknet/ | ✅ | 資産棚卸結果。添付取得も wiring 経由 |
| `portal` | 同上 | Favorites, MenuAccess, Dashboard, AccessRequests, UserManagement, NoticeManagement, DatabasePage, Bootstrap 等 | django/, persistence/ | ✅ | メニュー・お気に入り・管理画面。middleware は domain 定数で承認判定 |
| `identity` | 同上 | LoginPage, DesknetLogin 等 | desknet/, persistence/ | ✅ | 認証。`next` URL 検証は views |
| `sales` | （共有 ACL） | — | oracle/（接続 facade） | 部分 | 基幹 Oracle 接続の共有入口。業務 app はここ経由で import |
...
（リポジトリルート）
  ...
  src/                                    # Django ソース
    ...
    application/
      ...
      sales/                              # 営業（段階整備）
      # 各 app は §5.0 のレイヤー構成に従い、固有文書を docs/ に持つ
    templates/
    static/
```
...
| 0.10 | 2026-07-29 | 文書配置を SDD 構成へ移行。§3.4 の用語集配置を `src/application/<app>/docs/` + `src/docs/ubiquitous_language_core.md` に、§5.0・§5.1・§5.2 の構成表・ツリーを更新。0.7 で定めた「`Document/` ルート維持（`src/` 配下へは移さない）」のうち、**アプリ固有文書と横断設計文書は `src/` 配下へ移す**方針に改める（全体の手順書は `Document/` に維持） |
